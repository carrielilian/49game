...
> 最后更新：2026-07-17（结算函快照、预付币种对齐、ReadonlyCurrencyField）
...
| 第一行 | 最左 `BusinessTypeSelect`；中间 `ListSearchFields` 等；右侧 `aside` 放页级工具按钮（如【导出】） |
| 第二行 | `FilterBar.actions` 插槽；如游戏管理【添加游戏】 |
| 样式 | `style.css` → `.agf-list-toolbar__row` / `__aside` |

**游戏管理示例**：第一行 = 业务类型 + 游戏ID/名称 + 合同游戏名称 + 厂商ID + 厂商名称 + 【导出】；第二行 = 【添加游戏】。
...
| 厂商收入、游戏收入管理 — 预付分成款/已抵扣/剩余 | 取归属厂商 **支付币种** `Vendor.currency`（与【付款设置】预付区块 `CurrencyInput` 同源）；**未填**显示 `-` |
...
2. **合同编号** *  
3. **合同金额** * — 复合型输入框（`CurrencyInput`）；前缀随**归属厂商**「支付币种」显示 **￥** / **$**（在厂商管理维护）；≥0，精确至小数点后两位  
...
6. **补充说明**（多行，选填）  
7. **合作状态** *（单选：合作中 / 合作终止；默认合作中；保存同步游戏合作状态 + 操作日志）

**复合型输入框**：见 §5「复合型金额输入（CurrencyInput）」。

**校验**：合同编号、合同金额、合作内容（至少一项）、**合作状态**必填；已勾选合作内容对应的已付金额必填；失败 → 字段红字 + Toast「请完善所有信息」。

> **支付币种**在**厂商管理**添加/编辑厂商时维护（`Vendor.currency`）；合同抽屉金额前缀读取归属厂商币种。游戏/厂商级预付在【**付款设置**】维护。
...
| 预付分成款 | `Vendor.prepayment`（【付款设置】维护）；**未填** `-`；已填取 **`Vendor.currency`** |
| 已抵扣分成款 | 见下方计算口径；同厂商支付币种；**预付未填时 `-`** |
| 剩余未抵扣分成款 | 见下方计算口径；同厂商支付币种；**预付未填时 `-`** |
...
|------|------|
| 厂商ID / 厂商名称 | 只读 |
| 预付分成款 * | 必填；≥0；精确至小数点后两位；`CurrencyInput` 前缀取厂商「支付币种」 |
...
| 字段 | 规则 |
|------|------|
| 分成付款公司 * | 下拉；选项：`4399` / `纯游` / `纯游（美元）` / `香港4399` / `游家时代`（`SHARE_PAYMENT_COMPANY_OPTIONS`；**不含**游戏管理付款方中的「游乐」「游戏之家」） |
| 付款币种 * | 单选：人民币 / 美金 |
| 付款账号 * | 文本输入 |
...
| 付款方 * | 下拉选择；选项同游戏管理「付款方」：`4399` / `纯游` / `游乐` / `游戏之家` / `香港4399` / `游家时代`；必填；默认「请选择」；游戏付款未保存时默认取归属游戏 `payer` |
...
> **游戏付款管理**（`GamePaymentListPage`）【标记付款】/【详细信息】：只读 meta 为游戏ID/游戏名称；**分成付款公司**只读，取值 `Game.sharePaymentCompany`（游戏收入管理【付款设置】）；未配置时标记付款拦截；结算函传 `gameId` + `useGamePayments`。厂商付款管理仍保留「付款方」下拉。
...
| 预付分成款 | **未填** `-`；已填取归属厂商 **支付币种**（与【付款设置】预付区块一致） |
| 已抵扣 / 剩余未抵扣分成款 | 预付未填时与预付分成款一致显示 `-`；已填同厂商支付币种 |
| 操作 | 【付款设置】；余额 > 0 【申请付款】 |
...
**区块一：预付分成管理** — 预付分成款、历史已抵扣（`CurrencyInput`，前缀取归属厂商支付币种）；已抵扣/剩余只读（`ReadonlyCurrencyField`，同厂商支付币种；预付未填 `-`；历史/预付未保存时不显示 `0.00`）。

**区块二：付费设置** — 分成付款公司（下拉，4399 / 纯游 / 纯游（美元） / 香港4399 / 游家时代）、付款币种（人民币/美金 单选）、付款账号（必填）。游戏未保存时分成付款公司默认取游戏「付款方」（仅当付款方在上述五项内）。数据存 `Game.sharePayment*` / `Vendor.sharePayment*`。
...
与 **厂商付款管理** 同规则（730px `large`），差异如下：

| 差异 | 说明 |
|------|------|
| 只读 meta | 游戏ID、游戏名称（非厂商） |
| 分成付款公司 | **只读**（`ReadonlyField`）；取值 `Game.sharePaymentCompany`（游戏收入管理【付款设置】）；未配置时标记付款红字 + Toast 拦截 |
| 初始填充 | **每次打开**【标记付款】按 `utils/gamePaymentMarkDefaults.ts` 重算；汇率取 `ExchangeRateRecord` 中 **申请时间的上一个月** 月末值（`getExchangeRateByApplyTime`） |
| 金额输入 | 实际付款金额 `CurrencyInput`（￥）；实际付款美金 `CurrencyInput`（$）选填 |
| 数据字段 | `GamePaymentRequest.actualAmountUsd?`；保存时 `payBank` 写入当时分成付款公司快照 |
...
5. 邮箱  
6. 单位地址  
7. **支付币种** * — 下拉「人民币」/「美金」，默认**人民币**（第二行：单位地址 | 支付币种 | 发票信息）  
8. 发票信息 *  
...
校验：`VendorForm` → `validateVendorForm` / `VENDOR_REQUIRED`（**8 项**必填：厂商名称、**支付币种**、发票信息、银行 5 项；联系人/手机/邮箱/单位地址**选填**）。
...
27. **标记付款**：待付款金额只读（**￥**）；实际付款金额 `CurrencyInput`（**￥**）；**实际付款美金** `CurrencyInput`（**$**）选填；**厂商**付款方下拉（同游戏管理六项）；**游戏**付款为只读「分成付款公司」；收款信息必填；标记已付款关闭抽屉  
...
40. **预付分成管理输入**：预付/历史已抵扣用 `CurrencyInput`；前缀取厂商支付币种；精确至小数点后两位  
...
47. **厂商表单**：必填 8 项（含支付币种）；联系人/手机/邮箱/地址选填  
48. **内部渠道**：无 H5游戏（6 项）；外部渠道含**游乐外放**（5 项）  
49. **合同管理**：合同编号/金额/合作内容/合作状态必填；已付*随勾选；`CurrencyInput` 前缀取归属厂商币种；补充说明选填  
50. **厂商支付币种**：`Vendor.currency`；厂商表单第二行必填；合同抽屉只读继承  
...
64. **收入页结算币种**：账户余额/累计收入等结算口径列固定 **￥**；预付三列取 `Vendor.currency`；预付未填 `-`  
65. **付款列表币种**：待付/实付 ￥；与 Vendor.currency 无关  
...
72. **FilterBar 两行**：第一行查询 + `aside`（导出等）；第二行 `actions`（添加等）  
...
77. **标记付款付款方**：厂商下拉六项；**游戏**只读「分成付款公司」取 `Game.sharePaymentCompany`  
...
81. **厂商表单支付币种**：标签「支付币种」；`Vendor.currency`  
...
89. **游戏收入预付列币种**：列表三列取 `Vendor.currency`（非固定 ￥）  
