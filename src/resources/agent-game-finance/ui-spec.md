...
| 按钮 | 取消 / 确认申请 |
| 成功 | 绿 Toast「申请付款成功，账户余额已清零」；按申请时刻固定保存付款信息与结算函；在「游戏付款管理」**最上方**插入「未付款」记录；相关结算明细改「已申请」；余额清零 |
...
| 分类 | 内容 |
|------|------|
| 游戏 | 游戏编号、游戏名称、**支付币种**（申请时快照） |
| 厂商 | 编号、名称、银行收款信息 |
| 预付 | 预付分成款、剩余未抵扣预付分成款 |
| 付费设置 | **付款币种**、分成付款公司、付款账号 |
| 时间与金额 | 申请时间、待付款金额（= 申请时账户余额） |
| 结算函 | 整函快照（明细、合计、汇率、抵扣、初始实付等） |

后续源数据变更不影响本笔；**标记已付款**不更新结算函（含「实际付款金额」）。
...
|------|------|
| 取消 | 关闭，不保存 |
| 标记已付款 | 校验 → `markGamePaid`（**不**更新 `letterSnapshot`）→ **关闭**抽屉 → 绿 Toast「提交成功」 |
...
flowchart TB
  ...
  q1 -->|否| case1[→ 情况 1 无预付]
  q1 -->|是| q2{合同支付币种?}
  q2 -->|人民币| case2[→ 情况 2 有预付+合同人民币]
  q2 -->|美金| case3[→ 情况 3 有预付+合同美金]
  case1 --> write[写入输入框 · 保留两位小数]
  case2 --> write
  case3 --> write
...
| 情况 | 触发条件 | 实际付款金额（￥） | 实际付款美金（$） |
|:----:|---------|-------------------|------------------|
| 1 | 预付分成款 ≤ 0 | = 待付款金额 | 付款币种=美金时 = 待付÷汇率；否则空 |
| 2 | 预付分成款 > 0，合同支付币种 = 人民币 | = net | = net ÷ 汇率 |
| 3 | 预付分成款 > 0，合同支付币种 = 美金 | = usdNet × 汇率 | = usdNet |

- **net** = 待付款金额 − 剩余未抵扣分成款；若 net ≤ 0 则 net = 0  
- **usdNet** = 待付款金额 ÷ 汇率 − 剩余未抵扣分成款；若 usdNet ≤ 0 则 usdNet = 0  
- **合同支付币种** = `prepaymentCurrency` 快照优先，否则 `Contract.currency`

**Mock 验收**（`INITIAL_GAME_PAYMENTS`；申请时间 2026-07 → 汇率 **7.21**）

| 记录 | 情况 | 游戏 | 预期 |
|------|------|------|------|
| GP011 | 1 | 4009 | 实付=46,736.40；美金空 |
| GP012 | 1 | 4007 | 实付=30,875.00；美金≈4,282.25 |
| GP013 | 2 | 4003 | 实付≈110,610.80；美金≈15,341.30 |
| GP002 | 2 net=0 | 4001 | 实付=0.00；美金=0.00 |
| GP014 | 2 | 4010 | 实付=20,000.00；美金≈2,773.93 |
| GP015 | 3 | 4006 | 美金≈7,233.01；实付≈52,150.00 |
...
4. 退款明细；`合计④：`  
5. **`总计②-④：`**（= ② − ④，始终显示）  
6. **`汇率：`**（显示条件见「汇率行显示」；取申请月上月末汇率；四位小数）  

**汇率行显示**：付款币种为美金；或（支付币种（申请快照）为美金 且 申请付款时剩余 > 0）。剩余为申请时刻游戏收入列表「剩余未抵扣分成款」快照。

7. **（条件显示）** 仅当申请快照中「剩余未抵扣分成款」**> 0** 时，依次显示：
...
| 付款币种 | 取值 | 展示 |
|---------|------|------|
| 人民币 | **【申请付款】**时按三种情况算出并冻结（`letterSnapshot.letterPayAmount`） | **￥** |
| 美金 | 同上 | **$** |

> 与【标记付款】表单填写值无关；标记已付款**不**反写结算函实付。表单初始填充仍用 `resolveGameMarkPaymentDefaults`（每次打开重算）。

厂商付款管理仍用公式 `②−④−⑤`（无⑤时为 `②−④`）。

**结算函快照（游戏付款）**：【申请付款】成功时写入；打开只读，与付款状态无关。**标记已付款**不更新结算函（含「实际付款金额」）；表单实付仅写入付款记录。

**⑤ 取值**（均为【申请付款】时刻快照，写入结算函后不变）

| 项 | 说明 |
|----|------|
| 支付币种 | 申请付款那一刻的快照值 |
| 剩余未抵扣分成款 | 申请付款那一刻的快照值（与当时游戏收入列表一致） |
| 总计②−④ | 本函保存的合计 |
...
⑤ 与剩余行展示币种同**支付币种**。**汇率行**与**实际付款金额**仍按**付款币种**（【付款设置】）。

**汇率行显示**：付款币种为美金；或（支付币种（快照）为美金 且 申请付款时剩余 > 0）。

**显示条件**：仅当申请付款快照中剩余 > 0 时显示结算/抵扣金额与剩余行。

> **注意**：游戏付款结算函「实际付款金额」在**申请付款**时确定，标记已付款后不改变；与【标记付款】表单填写值无关（表单值用于付款记录/列表）。⑤ 两行仍基于 ②−④，与实付口径不同。
...
- **实际付款金额**（游戏）：读 `letterSnapshot.letterPayAmount`（申请时冻结）；**支付金额（大写）** 与之对应  
- **P001（1003）样例**：支付币种美金、付款币种人民币；**显示汇率行**（支付美金快照且申请时剩余>0）；总计②-④ **367,800**；结算/抵扣 **≈51,012.48**（$）；函内剩余 **≈818,987.52**（$）；结算函实付 **0**  
- **P002（1001）样例**：申请时剩余 ≤ 0 → 无结算/抵扣金额/剩余行；**总计②-④** + **实际付款金额**（人民币时无汇率行）  
- **GP001（4005）样例**：情况3（支付/付款均为美金）；`settlementIds=S004`；待付 **202,500**；总计②-④ **202,500**；结算/抵扣 **≈28,085.99**（$）；函内剩余 **≈326,914.01**（$）；结算函实付 **$0**（usdNet=待付÷汇率−剩余≤0）  
...
│   ├── settlementLetterSnapshot.ts # 【申请付款】时构建结算函快照（游戏）；厂商仍可按需重建
│   ├── exchangeRate.ts          # ExchangeRateRecord 查询；getExchangeRateByApplyTime
│   ├── gamePaymentMarkDefaults.ts # 游戏【标记付款】初始填充三种情况
...
32. **结算函 1175px**：按 `settlementIds` 过滤；连续月份合并；下载中文/英文；**总计②-④**、**实际付款金额** 始终显示；**汇率** 见 `resolveLetterShowExchangeRate`；申请快照剩余>0 才显示结算/抵扣与剩余行；游戏实付申请时冻结、标记不更新  
...
41. **结算函⑤**：按**支付币种**（申请快照）分支；申请快照剩余>0 才显示；汇率/实付仍看**付款币种**  
...
50. **合同支付币种**：`Contract.currency`；合同管理单选必填；各模块金额展示与输入前缀均读合同；**结算函⑤ 取申请快照**  
...
85. **游戏付款 mock**：GP011–GP015 覆盖三种情况 + GP002（情况2 net=0）；GP001 实付 $0  
86. **分成付款公司 vs 付款方**：游戏管理/厂商标记付款用 `GamePayer` 六项；付款设置/游戏标记付款用 `SharePaymentCompany` 五项  
87. **结算函快照**：游戏【申请付款】写入 `letterSnapshot` + `applySnapshot`；只读与付款状态无关；标记已付款**不**更新结算函（含实付）  
...
91. **游戏申请付款成功**：`applyGamePayment` 在 `gamePayments` **首行**插入未付款；冻结快照；Toast「申请付款成功，账户余额已清零」  
...
| 2026-07-22 | 游戏结算函「实际付款金额」申请时冻结；**标记已付款不更新**结算函 |
| 2026-07-22 | GP001 mock 修正（待付 202,500、实付 $0、⑤/剩余按 S004） |
| 2026-07-22 | 结算函批注：申请付款即固定快照（与付款状态无关）；⑤/汇率规则改业务语言 |
| 2026-07-22 | 【标记付款】自动填充由五种情况简化为三种；情况2 始终填美金 |
...
| 2026-07-17 | 结算函：汇率仅美金显示；游戏实付在申请时按三种情况冻结；⑤ 公式与实付口径区分说明 |
| 2026-07-17 | 结算函快照（游戏）：【申请付款】时写入 `letterSnapshot`；标记已付款**不**更新（厂商 mock 重建除外） |
